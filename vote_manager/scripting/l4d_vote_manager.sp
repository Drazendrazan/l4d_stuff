...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	if( test == Engine_Left4Dead ) bLeft4Dead2 = false;
	else if( test == Engine_Left4Dead2 ) bLeft4Dead2 = true;
	else
	{
...
public void OnPluginStart()
{
	...
	hCooldownMode = CreateConVar("l4d2_votemanager_cooldown_mode", "0", "0 = cooldown is shared 1 = cooldown is independant", CVAR_FLAGS);
	hVoteCooldown = CreateConVar("l4d2_votemanager_cooldown", "60.0", "Clients can call votes after this many seconds", CVAR_FLAGS);
...
stock Action ClientCanKick(int client, const char[] userid)
{
	...
	if(GetConVarBool(hTankImmunity) && IsPlayerAlive(target) && cTeam == 3)
	{
		char model[128];
...
stock void VoteManagerHandleCooldown(int client)
{
	...
	float cooldown = GetConVarFloat(hVoteCooldown);
